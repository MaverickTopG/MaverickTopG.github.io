...
const ACTIVE_TAB_GREEN = "#4D7C0F";
const CHART_HEIGHT = hp(22);
const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
const MS_PER_DAY = 24 * 60 * 60 * 1000;
const SCHEDULE_ACCENTS = ["#111827", "#0EA5E9", "#10B981", "#F59E0B"];
const BACKGROUND_TEXTURE_URI =
...
function shouldAutoCloseSheet(
  snapPoints,
  index,
  threshold = SHEET_AUTO_CLOSE_THRESHOLD,
) {
...
function withAlpha(hex = "#000000", alpha = 1) {
  ...
  if (Number.isNaN(intVal))
    return `rgba(0, 0, 0, ${Math.min(Math.max(alpha, 0), 1)})`;
...
function formatRangeLabel(start, end) {
  ...
  const endLabel = b.toLocaleDateString(
    "en-US",
    sameMonth
      ? { day: "numeric" }
      : {
...
function buildHeatmapForOffset(offset = 0, logs = [], filterLog = () => true) {
  ...
  for (let i = 0; i < cells.length; i += 7) {
    const slice = cells.slice(i, i + 7).map((dateObj) => {
      ...
      const appearance = resolveHeatmapAppearance(hours, {
        inMonth,
        isPast,
        isToday,
      });
  ...
  const totalMonthHours = Object.values(totals).reduce(
    (sum, val) => sum + val,
    0,
  );
...
const dedupeOrganizations = (orgs = []) => {
  ...
  return orgs.filter((org) => {
    ...
    const scopeId =
      extractGroupScopeIdFromRecord(org) || SUPER_ADMIN_GROUP_KEY;
    const dedupeKey = `${scopeId}::${code || ""}::${org?.linked_org_id || ""}::${org?.id || ""}`;
...
const findPreferredOrganizationByCode = (orgs = [], rawCode) => {
  const targetCode = normalizedCode(rawCode);
  if (!targetCode) return null;
  const matches = (Array.isArray(orgs) ? orgs : []).filter(
    (org) => normalizedCode(org?.access_code) === targetCode,
  );
  if (!matches.length) return null;
  const superAdminMatch =
    matches.find((org) => !extractGroupScopeIdFromRecord(org)) || null;
  return superAdminMatch || matches[0];
};

const isPersonalOrgRecord = (org = {}) => {
  const id = String(org?.id || "").toLowerCase();
  const code = String(
    org?.access_code || org?.organizationCode || org?.org_access_code || "",
  ).toLowerCase();
...
const isSchoolOrgRecord = (org = {}) => {
  ...
  const categoryKey = normalizePlanKey(
    org.category || org.type || org.org_type || org.organization_type,
  );
...
function resolveHeatmapAppearance(
  hours = 0,
  { inMonth = true, isPast = false, isToday = false } = {},
) {
  if (!inMonth) {
    return {
...
export default function VolunteerDashboard() {
  ...
  const blob1Shift = useSharedValue(0);
  const blob2Shift = useSharedValue(0);

  useEffect(() => {
    blob1Shift.value = withRepeat(
      withTiming(16, {
        duration: 5200,
  ...
  }, [blob1Shift, blob2Shift]);

  const bgBlobStyle1 = useAnimatedStyle(() => ({
    transform: [{ translateX: blob1Shift.value }, { translateY: blob1Shift.value }],
  }));
  const bgBlobStyle2 = useAnimatedStyle(() => ({
    transform: [{ translateX: blob2Shift.value }, { translateY: -blob2Shift.value }],
  }));
  ...
  const personalOrgId = useMemo(
    () => (user?.uid ? `personal-${user.uid}` : null),
    [user?.uid],
  );
  const personalOrg = useMemo(() => {
    ...
    return {
      ...
      targetGroupId: null,
      target_group_name: "Super Admin",
      targetGroupName: "Super Admin",
      sub_admin_group_id: null,
      subAdminGroupId: null,
      sub_admin_group_name: "Super Admin",
      subAdminGroupName: "Super Admin",
  ...
  const [isSubmittingDefaultShareRequest, setIsSubmittingDefaultShareRequest] =
    useState(false);

  const handleHistorySheetChange = useCallback(
    (index) => {
  ...
  const handleYearScheduleSheetChange = useCallback(
    (index) => {
      if (shouldAutoCloseSheet(sheetSnapPoints, index)) {
  ...
  const handleOrgInfoSheetChange = useCallback(
    (index) => {
  ...
  const handleAddOrgSheetChange = useCallback(
    (index) => {
      if (shouldAutoCloseSheet(sheetSnapPoints, index)) {
  ...
  const handleGoalSheetChange = useCallback(
    (index) => {
      if (shouldAutoCloseSheet(sheetSnapPoints, index)) {
  ...
  const handleDeleteOrgSheetChange = useCallback(
    (index) => {
  ...
  const handleDefaultShareSheetChange = useCallback(
    (index) => {
      if (shouldAutoCloseSheet(sheetSnapPoints, index)) {
  ...
  const [userOrgRequestRefs, setUserOrgRequestRefs] = useState({
    codeSet: new Set(),
    idSet: new Set(),
  });
    ...
    async (log, code) => {
      ...
      try {
        const volunteerEmail =
          log.email || userProfile?.email || user?.email || null;
        const volunteerName =
          `${log.firstName || userProfile?.firstName || ""} ${log.lastName || userProfile?.lastName || ""}`.trim() ||
          user?.displayName ||
          volunteerEmail ||
          "Volunteer";
        const payload = {
  ...
  const applyScannerZoom = useCallback(
  ...
  const scannerPinchGesture = useMemo(
    () =>
      Gesture.Pinch()
        .onStart(() => {
        ...
        .onUpdate((event) => {
          const proposedZoom =
            pinchBaseZoomRef.current + (event.scale - 1) * 0.35;
  ...
  const openAddModalFromQr = useCallback(
    (prefill) => {
      ...
      const matchedOrg = findPreferredOrganizationByCode(
        organizations,
        prefill.payload.orgAccessCode,
      );
      setSelectedOrganizations(
        matchedOrg?.id ? [String(matchedOrg.id)] : [PERSONAL_OPTION],
      );
      setModalStep(1);
      setNewSite("");
      setNewHours("");
    ...
    [organizations, resetAddSheetState],
  );
  ...
  const toggleOrganizationSelection = useCallback((value) => {
  ...
  const submitDefaultShareRequest = useCallback(async () => {
    ...
    try {
      ...
      const adminCandidates = Array.isArray(resolvedOrg?.admin_uids)
        ? resolvedOrg.admin_uids
        : [];
      const recipientSet = new Set(
        adminCandidates.filter(Boolean).map((uid) => String(uid)),
      );
      if (resolvedOrg?.owner_uid)
        recipientSet.add(String(resolvedOrg.owner_uid));
  ...
  async function resolveOrgByCode(code) {
    ...
    const cachedHasRecipients =
      cached &&
      Array.isArray(cached.admin_uids) &&
      cached.admin_uids.length > 0;
              ...
              : data.admin_uid
                ...
                : data.org_admin_uid
                  ...
                  : data.admin_user_ids
                    ? data.admin_user_ids
                    : [];
        const normalizedAdmins = adminCandidates
          .filter(Boolean)
          .map((uid) => String(uid));
  ...
  async function primeOrgCache(userOrgs = [], fetchedLogs = []) {
      ...
      if (o.access_code) {
        ...
        map[code] = {
          ...
          default_share_policy:
            o.default_share_policy ||
            o.defaultSharePolicy ||
    ...
    const ids = [
      ...new Set(fetchedLogs.map((l) => l.linked_org_id).filter(Boolean)),
    ];
    if (ids.length) {
      try {
        ...
        orgsSnap.forEach((s) => {
          ...
          const ownerCandidate =
            ...
            null;
          const rawCity =
            d.city ?? d.location_city ?? d.org_city ?? d.home_city ?? null;
          const rawState =
            d.state ?? d.location_state ?? d.org_state ?? d.home_state ?? null;
          const rawCountry =
            d.country ??
            d.location_country ??
            d.org_country ??
          ...
          map[code] = {
            ...
            category:
              d.category ?? d.focus ?? d.type ?? map[code]?.category ?? null,
  ...
  const buildOrganizationState = useCallback((docSnap) => {
    ...
    const category =
      ...
      null;
    const groupScopeId =
      extractGroupScopeIdFromRecord(data) ??
      extractGroupScopeIdFromRecord(cached || {});
    const groupScopeName = resolveGroupScopeNameFromRecord(
      data,
      groupScopeId,
    );
    const productIds = [
    ...
    return {
      ...
      default_share_status: isSchool
        ? "approved"
        : data.default_share_status || null,
      default_share_rejection_reason:
        data.default_share_rejection_reason || null,
      ...
      target_group_id: groupScopeId,
      targetGroupId: groupScopeId,
      target_group_name: groupScopeName,
      targetGroupName: groupScopeName,
      sub_admin_group_id: groupScopeId,
      subAdminGroupId: groupScopeId,
      sub_admin_group_name: groupScopeName,
      subAdminGroupName: groupScopeName,
      group_scope_id: groupScopeId,
      group_scope_name: groupScopeName,
    };
  }, []);
  ...
  const assertQrPayload = (payload) => {
    ...
    if (!QR_SUPPORTED_VERSIONS.has(Number(payload.v))) {
      throw new Error(
        "This QR version is not supported. Please update the app.",
      );
    }

    if (
      typeof payload.orgAccessCode !== "string" ||
      payload.orgAccessCode.trim().length < 3
    ) {
      throw new Error("Organization access code is invalid.");
    }
    if (
      typeof payload.adminId !== "string" ||
      payload.adminId.trim().length < 3
    ) {
  ...
  const normalizeQrPayload = (raw) => {
    ...
    const normalized = {
      ...raw,
      v: Number(raw.v),
      orgAccessCode: String(raw.orgAccessCode || "")
        .trim()
        .toUpperCase(),
  ...
  const handleBarcodeScanned = async ({ data }) => {
    ...
    try {
      ...
      const normalizedPayload = {
        ...
        orgAccessCode: String(unsignedPayload.orgAccessCode || "")
          .trim()
          .toUpperCase(),
      ...
      if (!verifyData.ok) {
        throw new Error(
          verifyData.message || "Signature could not be verified.",
        );
      }
      ...
      const idemRoot = await sha256Hex(
        `${userId}|${orgDocId || unsignedPayload.orgAccessCode}|${unsignedPayload.nonce}`,
      );

      const rootUsageSnap = await getDocs(
        query(
      ...
      if (!uniquePrepared) {
        throw new Error(
          "We could not prepare a fresh check-in. Please try again.",
        );
      }
      ...
      if (firebaseCode === "functions/unavailable") {
        message =
          "Verification service is unavailable. Check your connection and try again.";
      } else if (firebaseCode === "functions/not-found") {
        message =
          "QR verification service is not configured yet. Ask your admin to deploy it.";
  ...
  useEffect(() => {
      ...
      unsubscribeLogs = onSnapshot(
        ...
        async (snap) => {
          ...
          snap.forEach((docSnap) => {
            ...
            arr.push({
              ...
              linked_org_id: data.linked_org_id || null,
              target_group_id: extractGroupScopeIdFromRecord(data),
              target_group_name: resolveGroupScopeNameFromRecord(
                data,
                extractGroupScopeIdFromRecord(data),
              ),
              ...
              shared_default_codes: Array.isArray(data.shared_default_codes)
                ? data.shared_default_codes
                : [],
              is_personal:
                !!data.is_personal ||
                (personalOrgId && data.organization_id === personalOrgId),
            });
          });
          ...
          arr.forEach((e) => {
            ...
            if (!e.organization_name && e.linked_org_id)
              pendingIds.add(e.linked_org_id);
          });
          if (pendingCodes.size)
            await Promise.all(Array.from(pendingCodes).map(resolveOrgByCode));
          if (pendingIds.size) {
            try {
              const os = await getDocs(
                query(
                  collection(db, "users"),
                  where("__name__", "in", Array.from(pendingIds).slice(0, 10)),
                ),
              );
              os.forEach((s) => {
                const d = s.data();
          ...
          const finalArr = arr.map((e) => {
            const code = e.organization_id
              ? String(e.organization_id).toUpperCase()
              : null;
            if (!e.organization_name && code && orgCacheRef.current[code]) {
              const c = orgCacheRef.current[code];
              return {
                ...e,
                organization_name: c.name || e.organization_name,
                linked_org_id: e.linked_org_id || c.id || null,
              };
            }
            return e;
          });
        ...
        (err) => {
          ...
          if (isMounted.current) {
            Alert.alert(
              "Error",
              "Failed to load logs: " + (err?.message || err),
            );
            setLoadingLogs(false);
          }
        },
    ...
    const unsubscribeOrgs = onSnapshot(
      ...
      (snap) => {
        ...
        const dedupedOrgs = dedupeOrganizations(orgs).filter(
          (org) =>
            !isPersonalOrgRecord(org) &&
            !org?.is_archived &&
            !!(org?.access_code || org?.linked_org_id),
        );
        if (isMounted.current) {
      ...
      (err) => {
        ...
        Alert.alert(
          "Error",
          "Failed to load organizations in real-time: " + (err?.message || err),
        );
      },
    );
  ...
  async function fetchOrganizations() {
    try {
      ...
      Alert.alert(
        "Error",
        "Failed to load organizations: " + (err?.message || err),
      );
      return [];
    }
  }
  ...
  const isDeclinedJoinStatus = (status = "") => {
    const normalized = String(status || "").toLowerCase();
    return ["rejected", "declined", "denied", "canceled", "cancelled"].some(
    ...
    try {
      ...
      snap.forEach((docSnap) => {
        ...
        const normalizedOrgId =
          data.org_id || data.organization_id || data.linked_org_id || null;
      ...
      const existingScopedMemberships = new Set();
      existingSnap.forEach((docSnap) => {
        const info = docSnap.data() || {};
        if (info.access_code)
          existingCodes.add(String(info.access_code).toUpperCase());
        if (info.organizationCode)
          existingCodes.add(String(info.organizationCode).toUpperCase());
        if (info.linked_org_id)
          existingLinkedIds.add(String(info.linked_org_id));
        existingLinkedIds.add(docSnap.id);
        const infoScopeId =
          extractGroupScopeIdFromRecord(info) || SUPER_ADMIN_GROUP_KEY;
        const infoCode = normalizedCode(
          info.access_code || info.organizationCode || info.org_access_code,
        );
        const infoLinkedId = info.linked_org_id || info.org_id || info.organization_id || null;
        if (infoCode) {
          existingScopedMemberships.add(`${String(infoCode).toUpperCase()}::${infoScopeId}`);
        }
        if (infoLinkedId) {
          existingScopedMemberships.add(`${String(infoLinkedId)}::${infoScopeId}`);
        }
      });

      for (const req of accepted) {
        const { code, linkedOrgId, data, scopeId, scopeName } = req;
        const upperCode = code ? String(code).toUpperCase() : null;
        const normalizedLinkedId = linkedOrgId ? String(linkedOrgId) : null;
        const scopeKey = scopeId || SUPER_ADMIN_GROUP_KEY;
        const scopedCodeKey = upperCode ? `${upperCode}::${scopeKey}` : null;
        const scopedLinkedKey = normalizedLinkedId
          ? `${normalizedLinkedId}::${scopeKey}`
          : null;

        if (scopedCodeKey && existingScopedMemberships.has(scopedCodeKey))
          continue;
        if (scopedLinkedKey && existingScopedMemberships.has(scopedLinkedKey))
          continue;
        ...
        try {
          ...
          const payload = {
            ...
            status: "active",
            target_group_id: scopeId || null,
            targetGroupId: scopeId || null,
            target_group_name: scopeName || "Super Admin",
            targetGroupName: scopeName || "Super Admin",
            sub_admin_group_id: scopeId || null,
            subAdminGroupId: scopeId || null,
            sub_admin_group_name: scopeName || "Super Admin",
            subAdminGroupName: scopeName || "Super Admin",
            planKey: finalPlanKey || null,
            plan_key: finalPlanKey || null,
            default_share_policy:
              finalDefaultSharePolicy || (isSchoolOrg ? "required" : null),
          ...
          if (finalCode) {
            existingScopedMemberships.add(
              `${String(finalCode).toUpperCase()}::${scopeKey}`,
            );
          }
          if (finalLinkedId) {
            existingScopedMemberships.add(`${String(finalLinkedId)}::${scopeKey}`);
          }
        } catch (linkErr) {
  ...
  const promptJoinTargetSelection = useCallback((targets = [], orgLabel = "Organization") => {
    const availableTargets = Array.isArray(targets)
      ? targets
          .map((target) => ({
            id: String(target?.id || "").trim(),
            name: String(target?.name || "").trim() || "Subadmin Group",
            isSuperAdmin: !!target?.isSuperAdmin,
            alreadyJoined: !!target?.alreadyJoined,
          }))
          .filter((target) => target.id && !target.alreadyJoined)
      : [];

    if (!availableTargets.length) return Promise.resolve([]);
    if (availableTargets.length === 1) {
      return Promise.resolve([availableTargets[0].id]);
    }

    const scopedTargets = availableTargets.slice(0, 7);
    const allTargetIds = availableTargets.map((target) => target.id);
    return new Promise((resolve) => {
      const buttons = [
        ...scopedTargets.map((target) => ({
          text: target.isSuperAdmin
            ? "Main Org (Super Admin)"
            : target.name,
          onPress: () => resolve([target.id]),
        })),
      ];

      if (availableTargets.length > scopedTargets.length) {
        buttons.push({
          text: `Request all ${availableTargets.length} groups`,
          onPress: () => resolve(allTargetIds),
        });
      } else {
        buttons.push({
          text: "Request all listed groups",
          onPress: () => resolve(allTargetIds),
        });
      }
      ...
      Alert.alert(
        "Choose Join Group",
        `Select where to register in ${orgLabel}.`,
        buttons,
        {
          cancelable: true,
          onDismiss: () => resolve([]),
  ...
  const handleManualAccessSubmit = async () => {
    ...
    if (!code) {
      return Toast.show({
        type: "error",
        text1: "Missing code",
        text2: "Enter an organization access code.",
      });
    }
    ...
    try {
      ...
      if (!central) {
        ...
        return Toast.show({
          type: "error",
          text1: "Organization not found",
          text2: "No organization matches that code.",
        });
      }

      const getOrganizationJoinTargets = httpsCallable(
        functions,
        "getOrganizationJoinTargets",
      );
      const submitOrganizationJoinRequests = httpsCallable(
        functions,
        "submitOrganizationJoinRequests",
      );

      const joinTargetResp = await getOrganizationJoinTargets({
        accessCode: code,
      });
      const joinTargetData = joinTargetResp?.data || {};
      const joinTargets = Array.isArray(joinTargetData.targets)
        ? joinTargetData.targets
        : [];
      const openTargets = joinTargets.filter((target) => !target?.alreadyJoined);
      if (!openTargets.length) {
        ...
        return Toast.show({
          ...
          text2:
            "You already joined the main organization and all available subadmin groups.",
        });
      }

      const selectedGroupIds = await promptJoinTargetSelection(
        joinTargets,
        joinTargetData.orgName || central.name || code,
      );
      if (!selectedGroupIds.length) {
        return;
      }

      const normalizeScopeKey = (groupId) =>
        normalizeGroupScopeId(groupId) || SUPER_ADMIN_GROUP_KEY;
      const targetByScopeKey = new Map(
        joinTargets.map((target) => [
          normalizeScopeKey(target?.id),
          {
            id: String(target?.id || "").trim(),
            name: String(target?.name || "").trim() || "Subadmin Group",
            isSuperAdmin: !!target?.isSuperAdmin,
      ...
      const selectedScopeKeys = Array.from(
        new Set(
          selectedGroupIds
            .map((groupId) => normalizeScopeKey(groupId))
            .filter(Boolean),
        ),
      );

      const restoredScopeKeys = new Set();
      if (selectedScopeKeys.length) {
        const userOrgSnap = await getDocs(
          query(collection(db, "users"), where("user_id", "==", user.uid)),
        );
        for (const scopeKey of selectedScopeKeys) {
          const archivedMembershipDoc = userOrgSnap.docs.find((docSnap) => {
            const data = docSnap.data() || {};
            const docCode = normalizedCode(
              data.access_code || data.organizationCode || data.org_access_code,
            );
            const docLinkedId =
              data.linked_org_id || data.org_id || data.organization_id || null;
            const docScopeKey =
              extractGroupScopeIdFromRecord(data) || SUPER_ADMIN_GROUP_KEY;
            ...
            const isArchived =
              ...
              !!data.archived_at ||
              !!data.archived_by_user;
            const matchesCode = docCode && docCode === code;
            const matchesLinked =
              docLinkedId && String(docLinkedId) === String(central.id);
            return (
              isArchived &&
              (matchesCode || matchesLinked) &&
              docScopeKey === scopeKey
            );
          });
          if (!archivedMembershipDoc) continue;

          const target = targetByScopeKey.get(scopeKey);
          const scopedGroupId =
            scopeKey === SUPER_ADMIN_GROUP_KEY ? null : scopeKey;
          const scopedGroupName =
            target?.isSuperAdmin || scopeKey === SUPER_ADMIN_GROUP_KEY
              ? "Super Admin"
              : target?.name || "Subadmin Group";

          await updateDoc(archivedMembershipDoc.ref, {
            status: "active",
            archived_by_user: false,
            archived_at: null,
            removed_by_user: false,
            removed_at: null,
            name: central.name || code,
            access_code: code,
            organizationCode: code,
            org_access_code: code,
            linked_org_id: central.id || null,
            organization_id: central.id || null,
            org_id: central.id || null,
          ...
          restoredScopeKeys.add(scopeKey);
        }
      }

      const pendingGroupIds = selectedScopeKeys
        .filter((scopeKey) => !restoredScopeKeys.has(scopeKey))
        .map((scopeKey) => targetByScopeKey.get(scopeKey)?.id || scopeKey);

      let createdTargets = [];
      if (pendingGroupIds.length) {
        const submitResp = await submitOrganizationJoinRequests({
          accessCode: code,
          groupIds: pendingGroupIds,
        });
        const submitData = submitResp?.data || {};
        createdTargets = Array.isArray(submitData.createdTargets)
          ? submitData.createdTargets
          : [];
      }

      const restoredTargets = Array.from(restoredScopeKeys).map((scopeKey) => {
        const target = targetByScopeKey.get(scopeKey);
        const isSuperAdmin =
          scopeKey === SUPER_ADMIN_GROUP_KEY || !!target?.isSuperAdmin;
        return {
          id: target?.id || scopeKey,
          name: isSuperAdmin ? "Super Admin" : target?.name || "Subadmin Group",
          isSuperAdmin,
        };
      });
      ...
      [...restoredTargets, ...createdTargets].forEach((target) => {
        const scopeKey = normalizeScopeKey(target?.id);
        if (!scopeKey || seenTargetScopeKeys.has(scopeKey)) return;
        seenTargetScopeKeys.add(scopeKey);
      ...
      if (!effectiveTargets.length) {
        ...
        return Toast.show({
          type: "info",
          text1: "Already connected",
          text2: "No new groups were selected for this organization.",
        });
      }
      ...
      const targetLabel = effectiveTargets
        .map((target) =>
          target?.isSuperAdmin
            ? "Main Org"
            : String(target?.name || "Subadmin Group"),
        )
        .join(", ");
      const restoredCount = restoredTargets.length;
      const createdCount = createdTargets.length;
      Toast.show({
        type: "success",
        text1:
          restoredCount && !createdCount
            ? "Organization restored"
            : restoredCount
              ? "Restored and requested"
              : "Request sent",
        text2:
          restoredCount && !createdCount
            ? `Reactivated ${targetLabel}.`
            : `Requested ${targetLabel}. Waiting for approval.`,
      });
      closeAddOrgSheet();
    } catch (err) {
      ...
      Alert.alert(
        "Error",
        "Could not add organization: " + (err?.message || err),
      );
    } finally {
      if (isMounted.current) {
        setIsSubmittingOrgRequest(false);
  ...
  const openTopHeaderMenu = useCallback(() => {
    Alert.alert("Actions", "Choose an option", [
      { text: "Add Log", onPress: () => withHaptics(openAddModal, "medium")() },
      { text: "Download", onPress: () => withHaptics(handleExport, "medium")() },
      { text: "QR Code", onPress: () => withHaptics(openScanner, "medium")() },
      { text: "Sign out", style: "destructive", onPress: () => withHaptics(signOut, "heavy")() },
      { text: "Cancel", style: "cancel" },
    ]);
  }, [handleExport, openAddModal, openScanner, signOut]);
  ...
  const handleDeleteOrganization = async () => {
    if (!orgToDelete?.id) {
      return Toast.show({
        ...
        text2: "Select an organization to archive.",
      });
    }
    const accessCode = normalizedCode(
      orgToDelete?.access_code ||
        orgToDelete?.organizationCode ||
        orgToDelete?.org_access_code,
    );
    const linkedOrgId =
      orgToDelete?.linked_org_id ||
      orgToDelete?.organization_id ||
      orgToDelete?.org_id ||
      null;
    try {
      // archive any membership documents for this org (some users may have duplicates)
      const userOrgSnap = await getDocs(
        query(collection(db, "users"), where("user_id", "==", user.uid)),
      );
      const membershipArchives = [];
      userOrgSnap.forEach((docSnap) => {
        ...
        const matchesLinked =
          linkedOrgId &&
          docLinkedId &&
          String(docLinkedId) === String(linkedOrgId);
        if (matchesCode || matchesLinked || docSnap.id === orgToDelete.id) {
          membershipArchives.push(
      ...
      if (membershipArchives.length) {
        await Promise.allSettled(membershipArchives);
      } else {
        await updateDoc(doc(db, "users", orgToDelete.id), {
          status: "archived",
          archived_by_user: true,
          archived_at: serverTimestamp(),
          removed_by_user: true,
      ...
      setOrganizations((prev) =>
        dedupeOrganizations(prev.filter((org) => org.id !== orgToDelete.id)),
      );
      setSelectedOrganizations((prev) => {
        const removedId = String(orgToDelete?.id || "").trim();
        if (!removedId) return prev;
        return prev.filter((selectedId) => String(selectedId) !== removedId);
      });
      closeDeleteOrgSheet();
      await fetchOrganizations();
      Toast.show({
        type: "success",
        text1: "Archived",
        text2: "Organization hidden. You can unarchive it anytime.",
      });
    } catch (err) {
      Alert.alert(
        "Error",
        "Could not delete organization: " + (err?.message || err),
      );
    }
  };
  ...
  const handleNext = () => {
    if (!newSite.trim() || !newHours.trim())
      return Toast.show({
        ...
        text2: "Enter both task and hours.",
      });
    ...
    if (hrs > 24)
      return Toast.show({
        type: "error",
        text1: "Invalid Hours",
        text2: "You cannot log more than 24 hours in a single day.",
      });
    if (!qrPrefill && !selectedOrganizations.length)
      return Toast.show({
        type: "error",
        text1: "Select organization",
        text2: "Choose at least one organization before continuing.",
      });
  ...
  const handleSaveSession = async () => {
    ...
    const selectedOrgIds = normalizedSelections.filter(
      (value) => value !== PERSONAL_OPTION,
    );
    const isPersonalSelected = normalizedSelections.includes(PERSONAL_OPTION);
    const seenSelectedOrgIds = new Set();
    const selectedManualOrgs = selectedOrgIds
      .map((selectedValue) => {
        const byId = organizations.find(
          (org) => String(org?.id || "").trim() === selectedValue,
        );
        if (byId) return byId;
        // Backward compatibility for stale code-based selections.
        return findPreferredOrganizationByCode(organizations, selectedValue);
      })
      .filter((org) => {
        if (!org?.id) return false;
        if (seenSelectedOrgIds.has(org.id)) return false;
        seenSelectedOrgIds.add(org.id);
        return true;
      });
    ...
    try {
      if (currentQrPrefill) {
        const qrOrgCode = normalizedCode(currentQrPrefill.payload.orgAccessCode);
        const selectedQrOrg =
          selectedManualOrgs.find(
            (org) => normalizedCode(org?.access_code) === qrOrgCode,
          ) || findPreferredOrganizationByCode(organizations, qrOrgCode);
        const qrScopeId = extractGroupScopeIdFromRecord(selectedQrOrg || {});
        const qrScopeName = resolveGroupScopeNameFromRecord(
          selectedQrOrg || {},
          qrScopeId,
        );
        const qrOrgName =
          currentQrPrefill.orgName || selectedQrOrg?.name || qrOrgCode || null;
        const qrLinkedOrgId =
          currentQrPrefill.linkedOrgId ||
          selectedQrOrg?.linked_org_id ||
          selectedQrOrg?.organization_id ||
          selectedQrOrg?.org_id ||
          null;
        const qrApprovalTarget = qrScopeId
          ? `${qrOrgName || "Organization"} (${qrScopeName})`
          : qrOrgName || "Organization";
        const dupSnap = await getDocs(
          query(
            collection(db, LOGS_COLLECTION),
            where("user_id", "==", user.uid),
        ...
        const qrSession = {
          ...
          organization_name: qrOrgName,
          linked_org_id: qrLinkedOrgId,
          target_group_id: qrScopeId,
          targetGroupId: qrScopeId,
          target_group_name: qrScopeName,
          targetGroupName: qrScopeName,
          sub_admin_group_id: qrScopeId,
          subAdminGroupId: qrScopeId,
          sub_admin_group_name: qrScopeName,
          subAdminGroupName: qrScopeName,
          group_scope_id: qrScopeId,
          group_scope_name: qrScopeName,
          approve: `Awaiting ${qrApprovalTarget}'s approval`,
          source: "qr",
          tag: "checkedIn",
          scan_idempotency: currentQrPrefill.idemKey,
          scan_idempotency_root:
            currentQrPrefill.idemRoot || currentQrPrefill.idemKey,
        ...
        Toast.show({
          ...
          text2: `Sent to ${qrApprovalTarget} for approval.`,
        });
        closeAddModal();
        return;
      }

      if (!selectedManualOrgs.length) {
      ...
      const sentOrgNames = [];
      for (const selectedOrg of selectedManualOrgs) {
        const code = normalizedCode(
          selectedOrg?.access_code ||
            selectedOrg?.organizationCode ||
            selectedOrg?.org_access_code,
        );
        if (!code) continue;
        const scopeId = extractGroupScopeIdFromRecord(selectedOrg);
        const scopeName = resolveGroupScopeNameFromRecord(selectedOrg, scopeId);
        let orgName = selectedOrg?.name || code;
        let linkedOrgId =
          selectedOrg?.linked_org_id ||
          selectedOrg?.organization_id ||
          selectedOrg?.org_id ||
          null;
        if (code && (!orgName || !linkedOrgId)) {
          try {
            const central = await resolveOrgByCode(code);
            if (central) {
              orgName = central.name || orgName;
              linkedOrgId = central.id || linkedOrgId;
            }
          } catch (error) {
            console.warn("resolveOrgByCode failed", error);
          }
        }
        const approvalTarget = scopeId
          ? `${orgName} (${scopeName})`
          : orgName;
        const orgSession = {
          ...sessionBase,
          ...
          linked_org_id: linkedOrgId || null,
          target_group_id: scopeId,
          targetGroupId: scopeId,
          target_group_name: scopeName,
          targetGroupName: scopeName,
          sub_admin_group_id: scopeId,
          subAdminGroupId: scopeId,
          sub_admin_group_name: scopeName,
          subAdminGroupName: scopeName,
          group_scope_id: scopeId,
          group_scope_name: scopeName,
          approve: `Awaiting ${approvalTarget}'s approval`,
        };
        await addDoc(collection(db, LOGS_COLLECTION), orgSession);
        sentOrgNames.push(approvalTarget);
      }

      if (!sentOrgNames.length) {
        Toast.show({
          type: "error",
          text1: "Missing organization",
          text2: "Select a valid organization to submit your hours.",
        });
        return;
      }
  ...
  const isApprovedOrPersonal = useCallback(
    (log) => {
      ...
      if (
        typeof log.approve === "string" &&
        log.approve.toLowerCase().startsWith("approved by")
      )
        return true;
      return false;
    },
  ...
  const isDeclinedLog = useCallback(
    (log) => {
      ...
      return ["declined", "rejected", "denied", "canceled", "cancelled"].some(
      ...
      const eligibleLogs = logs.filter((log) => {
        ...
        const alreadyShared = new Set(
          Array.isArray(log.shared_default_codes)
            ? log.shared_default_codes
                .map((code) => normalizedCode(code))
                .filter(Boolean)
            : [],
        );
  ...
  useEffect(() => {
    ...
    subtitle =
      offset === 0 ? "" : `${offset} ${offset === 1 ? "week" : "weeks"} ago`;
    ...
    logs.forEach((entry) => {
      ...
      const idx = bounds.findIndex(
        (seg) => timestamp >= seg.startMs && timestamp <= seg.endMs,
      );
      if (idx !== -1) totals[idx] += hrs;
    });

    const highlightIdx = totals.length
    ...
    const maxSegmentGoal = chartPoints.reduce(
      (acc, point) => Math.max(acc, point.goal || 0),
      0,
    );
    ...
    const percent =
      periodGoal > 0
        ? Math.round(Math.min((periodTotal / periodGoal) * 100, 999))
        : 0;
    ...
    setPeriodSummary({
      ...
      topSegmentLabel:
        chartPoints[highlightIdx]?.displayLabel ||
        chartPoints[highlightIdx]?.label ||
        "",
    });
    ...
    prev.forEach((p, id) => {
      ...
      if (!n) {
        if (p.organization_name) {
          Alert.alert(
            "Log Update",
            `"${p.organization_name}" deleted your "${p.site}" log on "${p.date}".`,
          );
        }
        ...
        if (n.approve === "denied") {
          Alert.alert(
            "Log Denied",
            `Your log for "${n.site}" on ${n.date} was denied by ${n.organization_name || "the organization"}.`,
          );
        } else if (n.approve === "accepted") {
          Toast.show({
            type: "success",
            text1: "Log Approved!",
            text2: `Your log for "${n.site}" is now approved.`,
          });
        ...
        if (
          n.approve === "accepted" ||
          (typeof n.approve === "string" &&
            n.approve.toLowerCase().startsWith("approved by"))
        ) {
  ...
  const handleExport = async () => {
    if (logs.length === 0) {
      Toast.show({
        type: "info",
        text1: "No Data",
        text2: "There are no logs to export.",
      });
      return;
    }
    Toast.show({
    ...
    try {
      const dataForSheet = logs.map((log) => ({
        ...
        Organization:
          ...
          (personalOrgId && log.organization_id === personalOrgId
            ? "Personal"
            : null) ||
          (log.organization_id
            ? `Code: ${String(log.organization_id).toUpperCase()}`
            : "Unassigned"),
        "Approval Status": log.is_personal
          ? "Self Logged"
          : log.approve || "Pending",
      }));
      ...
      Toast.show({
        type: "error",
        text1: "Export Failed",
        text2: "Could not generate the report.",
      });
    }
  };
  ...
  const sortedLogs = useMemo(() => {
    const key = (e) => {
      if (e?.created_at) {
        ...
        if (typeof e.created_at?.toDate === "function")
          return e.created_at.toDate().getTime();
  ...
  const displayedLogs = useMemo(() => sortedLogs.slice(0, 3), [sortedLogs]);

  const highlightPoint = chartData[highlightIndex] || null;
  const highlightLabel =
    periodSummary.topSegmentLabel ||
    highlightPoint?.displayLabel ||
    highlightPoint?.label ||
    "No standout yet";
  const chartCeiling = useMemo(
    () =>
      Math.max(
        periodSummary.chartCeiling || 1,
        ...chartData.map((point) => Number(point.value) || 0),
        1,
      ),
  ...
  const comparisonStats = useMemo(() => {
    ...
    const averagePercent = Math.min(
      Math.round((averageHours / goal) * 100),
      150,
    );
  ...
  const lifetimeDonation = useMemo(
    () => Number(totalHours || 0) * DONATION_RATE,
    [totalHours],
  );
  ...
  const yearScheduleData = useMemo(() => {
    const months = Array.from({ length: 12 }, (_, offset) => {
      const monthData = buildHeatmapForOffset(
        offset,
        logs,
        isApprovedOrPersonal,
      );
  ...
  const totalHoursCardLabel = `${formatHours(totalHours || 0)}h`;
  const totalLogsCardLabel = `${Math.round(logs.length || 0)}`;
  ...
  const HeatmapDayCell = ({ day }) => {
    ...
    return (
      <Animated.View
        style={[
          styles.heatmapCell,
          {
            ...
            borderColor: day.borderColor || "transparent",
          },
      ...
      >
        {day.isToday ? (
          <Animated.View
            style={[styles.heatmapTodayPulse, { opacity: pulse }]}
            pointerEvents="none"
          />
        ) : null}
        ...
        {day.inMonth && day.hours > 0 ? (
          <Text style={[styles.heatmapCellHours, { color: day.textColor }]}>
  ...
  const renderLogItem = ({ item }) => {
    const accent = ACTIVE_TAB_GREEN;
    const code = item.organization_id
      ? String(item.organization_id).toUpperCase()
      : null;
    let orgName = item.organization_name || null;
    if (!orgName && code && orgCacheRef.current[code])
      orgName = orgCacheRef.current[code].name || null;

    const hours = Number(item.hours_contributed ?? item.hours) || 0;
    const isPersonalLog =
      !!item.is_personal ||
      (personalOrgId && code === String(personalOrgId).toUpperCase());
    const orgLabel = isPersonalLog
      ? ""
      : orgName || (code ? `Code: ${code}` : "Unassigned");
    ...
    const borderAccent = withAlpha(accent, 0.38);
    const strongAccent = APP_COLORS.secondary;

    return (
      <View
        style={[
          styles.logCard,
          { backgroundColor: softAccent, borderColor: borderAccent },
        ]}
      ...
      >
        <View style={styles.logHeaderRow}>
          ...
          >
            <Text
              style={[styles.logHoursText, { color: strongAccent }]}
            >{`${formatHours(hours)}h`}</Text>
          </View>
        </View>
        ...
        <View style={styles.logMetaRow}>
          ...
          {!isPersonalLog && orgLabel ? (
            <>
              <View style={styles.logMetaDot} />
              <Text
                style={[styles.logOrg, { color: strongAccent }]}
                numberOfLines={1}
              >
                {orgLabel}
              </Text>
  ...
  const renderOrganizationItem = ({ item }) => {
    ...
    return (
      <View
        style={[
          styles.orgGridCard,
          { borderColor: withAlpha(accent, 0.35), backgroundColor: "#FFFFFF" },
        ]}
      >
        <View
          style={[
            styles.orgGridAvatar,
            { backgroundColor: withAlpha(accent, 0.16) },
          ]}
        >
          <Text style={[styles.orgGridInitials, { color: accent }]}>
            {getInitials(item.name)}
          </Text>
        </View>
        <View style={styles.orgGridDetail}>
          <Text style={styles.orgGridName} numberOfLines={1}>
            {item.name}
          </Text>
  ...
  return (
    <View style={styles.container}>
      <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
        <Reanimated.View style={[styles.blob1, bgBlobStyle1]} />
        <Reanimated.View style={[styles.blob2, bgBlobStyle2]} />
        <ImageBackground
          source={{ uri: BACKGROUND_TEXTURE_URI }}
      ...
      >
        <View style={styles.topHeaderRow}>
          <View style={styles.topHeaderTitleWrap}>
            <Text style={styles.topHeaderTitle}>Volunteer</Text>
            <Text style={styles.topHeaderSubtitle}>Log.</Text>
          </View>
          {Platform.OS === "ios" ? (
            <Host style={styles.topHeaderGearHost}>
              <ContextMenu activationMethod="singlePress">
                <ContextMenu.Items>
                  ...
                  <SwiftUIButton
                    systemImage="qrcode"
                    onPress={() => withHaptics(openScanner, "medium")()}
                  >
                    QR Code
                  </SwiftUIButton>
                  <SwiftUIButton
                    ...
                    onPress={() => withHaptics(signOut, "heavy")()}
                  >
                    Sign out
                  </SwiftUIButton>
                </ContextMenu.Items>
                <ContextMenu.Trigger>
                  <View style={styles.gearIconGlassContainer}>
                    <BlurView
                      intensity={80}
                      tint="light"
                      style={styles.gearIconGlassBlur}
                    >
                      <Gear size={wp(5.2)} color="#0F172A" weight="bold" />
                    </BlurView>
                  </View>
                </ContextMenu.Trigger>
              </ContextMenu>
            ...
            <TouchableOpacity
              onPress={withHaptics(openTopHeaderMenu, "medium")}
              activeOpacity={0.9}
              style={styles.topHeaderGearHost}
            >
              <View style={[styles.gearIconGlassContainer, styles.gearIconFallback]}>
                <Gear size={wp(5.2)} color="#0F172A" weight="bold" />
              </View>
            </TouchableOpacity>
          )}
          ...
          >
            <View style={styles.heroTitleRow}>
              <Text style={styles.heroTitle} numberOfLines={1}>
                {periodSummary.title || "Volunteer Impact"}
              </Text>
            </View>
            ...
            <View style={styles.periodNavRow}>
              ...
              <Text style={styles.periodNavLabel}>
                {periodSummary.label ||
                  (periodSummary.activeOffset === 0 ? "Current period" : "")}
              </Text>
              <TouchableOpacity
                style={[
                  styles.periodNavButton,
                  !periodSummary.canGoForward && styles.periodNavButtonDisabled,
                ]}
        ...
        <View style={styles.summaryStatsSection}>
          <View style={styles.summaryStatsGrid}>
            <View style={[styles.summaryStatCard, styles.summaryCardShadow]}>
              <View style={styles.summaryStatIconWrapper}>
                <Clock size={wp(5)} color="#111827" weight="bold" />
              </View>
              <Text style={styles.summaryStatValue}>{totalHoursCardLabel}</Text>
              <Text style={styles.summaryStatLabel}>Total Hours</Text>
            </View>
            <View style={[styles.summaryStatCard, styles.summaryCardShadow]}>
              <View style={styles.summaryStatIconWrapper}>
                <Checks size={wp(5)} color="#111827" weight="bold" />
              </View>
              <Text style={styles.summaryStatValue}>{totalLogsCardLabel}</Text>
        ...
        <View className="progressSection" style={styles.progressSection}>
          ...
          <View style={styles.progressChartContainer}>
            <Animated.View
              style={[
                styles.progressBarsRow,
                styles.progressBarsRowWeekly,
            ...
            >
              {chartData.map((point, idx) => {
                ...
                const barRatio =
                  chartCeiling > 0 ? Math.min(value / chartCeiling, 1) : 0;
                ...
                return (
                  ...
                  >
                    <Text
                      style={[
                        styles.progressBarTopValue,
                        isZero && styles.progressBarTopValueZero,
                      ]}
                      numberOfLines={1}
                    >
                      {formatHours(value)}
                    ...
                    <Text
                      style={[
                        ...
                        isActive && styles.progressBarLabelActive,
                      ]}
                    >
                      {point.label}
                    </Text>
                    {point.subLabel ? (
                      <Text
                        style={styles.progressBarSubLabel}
                        numberOfLines={1}
                      >
                        {point.subLabel}
                      </Text>
        ...
        <View style={styles.heatmapCard}>
          <View style={styles.heatmapHeaderRow}>
            <Text style={styles.heatmapTitle}>
              {heatmapCalendar.monthLabel || "Schedule"}
            </Text>
            <TouchableOpacity
          ...
          <View style={styles.heatmapWeekdayRow}>
            {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((label) => (
              <Text key={`weekday-${label}`} style={styles.heatmapWeekdayLabel}>
                {label}
              </Text>
            ))}
          </View>
          ...
          <View style={styles.heatmapGrid}>
            {heatmapCalendar.weeks.map((week, weekIdx) => (
              <View
                key={`heatmap-week-${weekIdx}`}
                style={styles.heatmapWeekRow}
              >
                {week.map((day) => (
          ...
          {!heatmapCalendar.hasActivity ? (
            <Text style={styles.heatmapEmptyText}>
              No sessions tracked for this month yet.
            </Text>
          ) : null}
        </View>
          ...
          {loadingLogs ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                Tap the plus button below to add your first session.
              </Text>
            </View>
          ) : (
            <FlatList
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity
              ...
              onPress={withHaptics(openAddOrgSheet, "medium")}
              activeOpacity={0.85}
            >
          ...
          {organizations.length === 0 ? (
            <View style={styles.emptyOrgContainer}>
              ...
              <Text style={styles.emptyOrgSubtitle}>
                Tap the plus icon to link your first partner.
              </Text>
            </View>
          ) : (
            <FlatList
      ...
      >
        <View style={styles.bottomSheetHeader}>
          ...
          <TouchableOpacity
            onPress={withHaptics(closeHistorySheet, "light")}
      ...
      >
        <BottomSheetScrollView
          contentContainerStyle={styles.yearScheduleContent}
        >
          <View style={styles.bottomSheetHeader}>
            <Text style={styles.bottomSheetTitle}>Yearly Schedule</Text>
            <TouchableOpacity
              onPress={withHaptics(closeYearScheduleSheet, "light")}
              style={styles.modalClose}
            >
              <X size={wp(6)} color="#666" />
            </TouchableOpacity>
          ...
          {yearScheduleData.months.map((month, idx) => (
            ...
            >
              <View style={styles.yearScheduleSectionHeader}>
                <Text style={styles.yearScheduleSectionTitle}>
                  {month.label}
                </Text>
                <Text style={styles.yearScheduleSectionHours}>
                  {month.totalHours
                    ? `${formatHours(month.totalHours)}h logged`
                    : "0h logged"}
                </Text>
              </View>
              <View style={styles.yearScheduleWeekdayRow}>
                {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((label) => (
                  <Text
                    key={`weekday-${label}`}
                    style={styles.yearScheduleWeekdayLabel}
                  >
                    {label}
                  </Text>
                ))}
              ...
              <View style={styles.yearScheduleGrid}>
                {month.weeks.map((week, weekIdx) => (
                  <View
                    key={`${month.key}-week-${weekIdx}`}
                    style={styles.yearScheduleWeekRow}
                  >
                    {week.map((day) => (
                      <View
                        key={day.key}
              ...
              {!month.hasActivity ? (
                <Text style={styles.yearScheduleEmpty}>
                  No sessions tracked for this month yet.
                </Text>
              ) : null}
            </View>
          ...
          {!yearScheduleData.hasActivity ? (
            <Text style={styles.yearScheduleHint}>
              Add sessions to see them organized by month.
            </Text>
          ) : null}
        </BottomSheetScrollView>
      </BottomSheet>
            ...
            {modalStep === 1 ? (
              <>
                <Text style={styles.modalDescription}>
                  Capture what you worked on and how long you contributed.
                </Text>
              ...
              <View style={styles.qrOrgSummary}>
                <View style={styles.qrOrgHeader}>
                  ...
                  <View style={styles.qrOrgHeaderText}>
                    <Text style={styles.qrOrgSummaryTitle}>
                      Secure QR Check-In
                    </Text>
                    <Text style={styles.qrOrgSummarySubtitle}>
                      This session routes privately to{" "}
                      {qrPrefill.orgName || qrPrefill.payload.orgAccessCode}.
                      All admin validation runs on our servers.
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.qrOrgSummaryNote}>
                  ...
                  <Text style={styles.qrOrgSummaryNoteText}>
                    Sensitive access codes stay hidden. We encrypt them and
                    process the approval flow in the background for you.
                  </Text>
                </View>
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={styles.qrForgetButtonText}>
                    Forget this check-in
                  </Text>
                </TouchableOpacity>
                <Text style={styles.qrForgetHint}>
                  You will need to rescan the QR code if you want to submit it
                  later.
                </Text>
              </View>
            ) : (
              <>
                ...
                <Text style={styles.organizationHelperText}>
                  Enter the access code your organization admin shared to send a
                  join request.
                </Text>
                  ...
                  {organizations.length === 0 ? (
                    ...
                    <>
                      {organizations.map((org, idx) => {
                        const accent =
                          SCHEDULE_ACCENTS[idx % SCHEDULE_ACCENTS.length];
                        const selectionValue = String(org?.id || "").trim();
                        if (!selectionValue) return null;
                        const scopeId = extractGroupScopeIdFromRecord(org);
                        const scopeName = resolveGroupScopeNameFromRecord(
                          org,
                          scopeId,
                        );
                        const isSelected =
                          selectedOrganizations.includes(selectionValue);
                        return (
                          <TouchableOpacity
                            key={org.id}
                            style={[
                            ...
                            onPress={withHaptics(
                              () =>
                                toggleOrganizationSelection(selectionValue),
                              "medium",
                            )}
                            activeOpacity={0.88}
                          >
                            <View style={styles.orgVerticalContent}>
                              ...
                              <Text
                                style={[
                                  ...
                                  isSelected
                                    ? styles.orgVerticalTextSelected
                              ...
                              {scopeId ? (
                                <View
                                  style={[
                                    styles.orgScopePill,
                                    isSelected
                                      ? styles.orgScopePillSelected
                                      : null,
                                  ]}
                                >
                                  <Text
                                    style={[
                                      styles.orgScopePillText,
                                      isSelected
                                        ? styles.orgScopePillTextSelected
                                        : null,
                                    ]}
                                  >
                                    {scopeName}
                                ...
                                >
                                  <Text
                                    style={[
                                      ...
                                      isSelected
                                        ? styles.orgDefaultPillTextSelected
                                        : null,
                                    ]}
                                  >
                                    Default
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      selectedOrganizations.includes(PERSONAL_OPTION) &&
                        styles.orgVerticalButtonNeutralActive,
                    ]}
                ...
                <TouchableOpacity
                  ...
                  onPress={withHaptics(() => {
                    ...
                    if (qrPrefill && qrPrefill.payload?.orgAccessCode) {
                      const matchedQrOrg = findPreferredOrganizationByCode(
                        organizations,
                        qrPrefill.payload.orgAccessCode,
                      );
                      setSelectedOrganizations([
                        matchedQrOrg?.id
                          ? String(matchedQrOrg.id)
                          : PERSONAL_OPTION,
                      ]);
                    } else {
                      setSelectedOrganizations([PERSONAL_OPTION]);
      ...
      >
        ...
        >
          <View style={styles.infoSheetHeader}>
            <Text style={styles.infoSheetTitle}>
          ...
          <Text style={styles.infoSheetParagraph}>
            Organizations are groups (schools, clubs, nonprofits) that review
            and approve your volunteer hours. When you connect with one, your
            sessions can be sent directly to their admin dashboard.
          </Text>
          <Text style={styles.infoSheetParagraph}>
            Example: Your school's Peer Tutoring Club gives you an access code.
            Enter that code, confirm it matches the club name, then send a join
            request. The admins will be notified automatically.
          </Text>
          <Text style={styles.infoSheetParagraph}>
            After an admin approves you, any sessions you submit to that
            organization will appear in their approval queue—no extra emails or
            spreadsheets required.
          </Text>
          <Text style={styles.infoSheetParagraph}>
            Some school organizations require all of your hours to be visible
            once you join. Other organizations stay manual and will only see
            hours you choose to submit to them.
          </Text>
        </BottomSheetScrollView>
      </BottomSheet>
      ...
      >
        ...
        >
          ...
          <View style={styles.modalBody}>
            <Text style={styles.modalDescription}>
              Enter the organization access code provided by an admin to send
              your join request. We'll verify it automatically.
            </Text>
            <View style={styles.manualAccessContainer}>
              ...
              {manualResolveState === "searching" && (
                <Text style={styles.manualResolveStatus}>
              ...
              {manualResolveState === "found" && manualResolveName && (
                <Text
                  style={[
                    styles.manualResolveStatus,
                    styles.manualResolveStatusSuccess,
                  ]}
                >
              ...
              {manualResolveState === "not_found" && (
                <Text
                  style={[
                    styles.manualResolveStatus,
                    styles.manualResolveStatusError,
                  ]}
                >
          ...
          <View style={styles.modalActions}>
            ...
            <TouchableOpacity
              style={[
                ...
                manualActionDisabled && { opacity: 0.6 },
              ]}
      ...
      >
        ...
        >
          ...
          <View style={styles.modalBody}>
            <Text style={styles.modalDescription}>
              Sharing requests are currently disabled in this app. If you need
              help, contact your organization admin directly.
            </Text>
          ...
          <View style={styles.modalActions}>
            ...
            <TouchableOpacity
              style={[
                styles.modalSaveButton,
                isSubmittingDefaultShareRequest && { opacity: 0.6 },
              ]}
              onPress={withHaptics(submitDefaultShareRequest, "medium")}
              disabled={
                isSubmittingDefaultShareRequest || !defaultShareTargetOrg?.id
              }
            >
              {isSubmittingDefaultShareRequest ? (
                <ActivityIndicator size="small" color="#FFF" />
      ...
      >
        <BottomSheetScrollView
          contentContainerStyle={[
            ...
            styles.deleteSheetContent,
          ]}
          keyboardShouldPersistTaps="handled"
        >
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Archive Organization</Text>
            <TouchableOpacity
          ...
          <View style={styles.modalBody}>
            <Text style={styles.deleteWarningText}>
              Archive &quot;{orgToDelete?.name || "this organization"}&quot;?
              This will hide it from your Volunteer Log, Messages, and Events.
            </Text>
            <Text style={styles.deleteWarningText}>
              You can bring it back anytime by entering the same access code to
              unarchive it.
            </Text>
          </View>
            ...
            <>
              ...
              <View pointerEvents="none" style={styles.scannerFocusWrap}>
                <View style={styles.scannerFocusBox}>
                  <View
                    style={[styles.scannerCorner, styles.scannerCornerTL]}
                  />
                  <View
                    style={[styles.scannerCorner, styles.scannerCornerTR]}
                  />
                  <View
                    style={[styles.scannerCorner, styles.scannerCornerBL]}
                  />
                  <View
                    style={[styles.scannerCorner, styles.scannerCornerBR]}
                  />
                </View>
              </View>
                ...
                <View style={styles.scannerCaptionCard}>
                  <View
                    style={[
                      ...
                      styles[
                        `scannerStatusPill__${scanFeedback.tone || "neutral"}`
                      ],
                    ]}
                  >
                  ...
                  <Text style={styles.scannerCaptionTitle}>
                    {scanFeedback.title}
                  </Text>
                  <View style={styles.scannerCaptionRow}>
                    {scanFeedback.tone === "busy" && (
                      <ActivityIndicator
                        size="small"
                        color="#6366F1"
                        style={{ marginRight: 8 }}
                    ...
                    <Text style={styles.scannerCaptionNote}>
                      {scanFeedback.note}
                    </Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  bottomSheetTitle: { fontSize: wp(4), fontWeight: "700", color: APP_COLORS.secondary },
  bottomSheetContent: {
  ...
  progressTitle: { fontSize: wp(4.8), fontWeight: "800", color: APP_COLORS.secondary },
  progressMeta: {
    marginTop: hp(0.4),
    fontSize: wp(3.1),
  ...
  goalPillText: { fontSize: wp(3.2), fontWeight: "600", color: APP_COLORS.secondary },
  progressGoalPill: {
  ...
  progressBarLabelActive: { color: APP_COLORS.secondary },
  progressBarSubLabel: { fontSize: wp(2.6), color: "rgba(17,24,39,0.35)" },
  heatmapCard: {
    marginTop: hp(3.4),
    backgroundColor: APP_COLORS.white,
    borderRadius: wp(10),
  ...
  logHoursText: { fontSize: wp(3.4), fontWeight: "700", color: APP_COLORS.secondary },
  logProgressLabel: {
    fontSize: wp(3),
    color: "rgba(17,24,39,0.55)",
    fontWeight: "600",
  ...
  logMeta: { fontSize: wp(3.1), color: "rgba(17,24,39,0.6)" },
  logMetaDot: {
    width: wp(1.5),
    height: wp(1.5),
  ...
  orgGridName: { fontSize: wp(3.6), fontWeight: "700", color: APP_COLORS.secondary },
  orgGridCode: {
    fontSize: wp(3),
    color: "rgba(17,24,39,0.55)",
    marginTop: hp(0.2),
  ...
  modalHeader: {
    ...
    borderBottomWidth: 1,
    borderBottomColor: APP_COLORS.glassBorder,
  },
  modalHeaderActions: {
    flexDirection: "row",
    alignItems: "center",
  ...
  qrOrgSummary: {
    ...
    borderWidth: 1,
    borderColor: "rgba(210, 246, 119, 0.4)",
    gap: hp(1.6),
  },
  qrOrgHeader: {
    flexDirection: "row",
  ...
  qrOrgIcon: {
    ...
    height: wp(10),
    borderRadius: wp(3.2),
    backgroundColor: "rgba(210, 246, 119, 0.35)",
    alignItems: "center",
    justifyContent: "center",
  },
  qrOrgHeaderText: {
  ...
  orgScopePill: {
    marginLeft: wp(1),
    paddingHorizontal: wp(2.4),
    paddingVertical: wp(0.45),
    borderRadius: 999,
    backgroundColor: "rgba(59,130,246,0.14)",
  },
  orgScopePillSelected: {
